...
st.markdown(
    "<h1 style='text-align: center;'>📦 Báo Cáo Đơn Hàng TikTok & Shopee</h1>",
    unsafe_allow_html=True,
)

col1, col2 = st.columns(2)
with col1:
    file_tiktok = st.file_uploader("📥 Upload file TikTok", type=["xlsx", "xls"])

with col2:
    file_shopee = st.file_uploader("📥 Upload file Shopee", type=["xlsx", "xls"])
...
if process_btn:
    if not file_tiktok or not file_shopee:
        ...
        bang_thong_ke_so_luong_shopee = pd.DataFrame(
            {
                "SC_Combo": [
                    (so_luong_SC_Combo_sp_hoanh_thanh + so_luong_SC_Combo_sp_da_giao)
                    * 2
                ],
        ...
        st.subheader("📊 Bảng Thống Kê Đơn Hàng Hoàn Trả Tiktok & Shopee")
        st.dataframe(bang_thong_ke_don_hang)

        st.subheader("📊 Bảng Thống Kê SL Đơn Hàng Hoàn Trả Tiktok & Shopee")
